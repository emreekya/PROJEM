...
class TestVideoService(unittest.TestCase):
    ...
    def test_replace_ordered_clip_prefix_preserves_first_scene_remainder(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            trimmed = os.path.join(temp_dir, "_intro-trimmed-scene-1.mp4")
            for file_path in (intro, first, second):
                Path(file_path).write_bytes(b"clip")

            def fake_trim(input_path, out_path, start_seconds):
                self.assertEqual(input_path, first)
                self.assertAlmostEqual(start_seconds, 2.2)
                Path(out_path).write_bytes(b"trimmed")
                return True

            with patch.object(vd, "clip_duration_seconds", side_effect=[7.5]), patch.object(
                vd, "_trim_clip_start", side_effect=fake_trim
            ):
                result = vd.replace_ordered_clip_prefix(
                    [first, second], intro, 2.2, temp_dir
                )

            self.assertEqual(result, [intro, trimmed, second])
