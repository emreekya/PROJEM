...
class TestScriptPromptOptions(unittest.TestCase):
    def test_script_repetition_reason_rejects_phrase_loop(self):
        repeated = (
            "Bu top ve mermiler sehri ele gecirmek icin buyuk rol oynadi. "
            * 6
        )

        self.assertTrue(llm.script_repetition_reason(repeated))

    def test_script_repetition_reason_accepts_varied_script(self):
        varied = " ".join(
            [
                "Fatih Sultan Mehmet kusatma planini dikkatle hazirladi.",
                "Gemiler karadan yurutilerek Halic tarafina indirildi.",
                "Toplar surlarin belirli bolgelerine yonlendirildi.",
                "Savunma hatlari gunler ilerledikce baski altinda kaldi.",
                "Son hucumla birlikte sehirde yeni bir donem basladi.",
            ]
        )

        self.assertEqual(llm.script_repetition_reason(varied), "")
    ...
    def test_numbered_title_requires_distinct_visual_points(self):
        prompt = llm.build_script_prompt(
            video_subject="İkizler burcunun pek bilinmeyen 5 özelliği",
            language="tr",
            paragraph_number=3,
        )

        self.assertIn("deliver exactly 5 distinct", prompt)
        self.assertIn("Prefer concrete, visually depictable statements", prompt)
        self.assertIn("Avoid repetition", prompt)
        self.assertIn("explicitly signal every point", prompt)
        self.assertIn("Do not add historical, mythological or religious background", prompt)
        self.assertIn("Avoid unsupported absolute claims", prompt)
        self.assertIn('write dates such as "21 Mayıs"', prompt)
        self.assertIn("Do not use insulting, stigmatizing", prompt)
    ...
    def test_cover_prompt_requests_topic_specific_editorial_direction(self):
        captured = {}

        def fake_generate_response(prompt):
            captured["prompt"] = prompt
            return "a clean modern robotics laboratory, premium editorial lighting"

        with patch.object(llm, "_generate_response", side_effect=fake_generate_response):
            result = llm.generate_cover_image_prompt("Yapay zeka robotları")

        self.assertIn("robotics laboratory", result)
        self.assertIn("Infer the correct visual domain", captured["prompt"])
        self.assertIn("unless the topic explicitly requires them", captured["prompt"])

    def test_cover_prompt_uses_safe_fallback_when_llm_fails(self):
        with patch.object(llm, "_generate_response", side_effect=RuntimeError("offline")):
            result = llm.generate_cover_image_prompt("Kahve demleme teknikleri")

        self.assertIn("Kahve demleme teknikleri", result)
        self.assertIn("contemporary editorial visual", result)

    def test_terms_prompt_requires_concrete_visual_variety(self):
        captured = {}

        def fake_generate_response(prompt):
            captured["prompt"] = prompt
            return '["Gemini constellation", "friends talking cafe"]'

        with patch.object(llm, "_generate_response", side_effect=fake_generate_response):
            result = llm.generate_terms("İkizler burcu", "iletişim ve yaratıcılık", amount=2)

        self.assertEqual(result, ["Gemini constellation", "friends talking cafe"])
        self.assertIn("DIFFERENT concrete visual concept", captured["prompt"])
        self.assertIn("do not return near-synonyms", captured["prompt"])
        self.assertIn("grammatically correct English search phrases", captured["prompt"])

    def test_scene_prompt_discards_sensitive_drift_for_normal_topic(self):
        unsafe = (
            "a medieval warrior holding a sword in front of a mosque, "
            "dramatic fantasy composition"
        )

        result = llm._sanitize_generated_image_prompt(
            unsafe,
            "İkizler burcu insanları yaratıcı olabilir",
            "İkizler burcunun bilinmeyen 5 özelliği",
        )

        self.assertIn("ordinary contemporary setting", result)
        self.assertIn("İkizler burcu insanları yaratıcı olabilir", result)
        self.assertNotIn("warrior", result)
        self.assertNotIn("sword", result)
        self.assertNotIn("mosque", result)

    def test_scene_prompt_keeps_required_historical_direction(self):
        historical = "Ottoman soldiers approaching the city walls in 1453"

        result = llm._sanitize_generated_image_prompt(
            historical,
            "Fatih Sultan Mehmet İstanbul'un fethini planlıyor",
            "Fatih Sultan Mehmet'in İstanbul'u fethi",
        )

        self.assertIn("Ottoman soldiers", result)
        self.assertIn("directly required by the narration", result)
